...
// Constructor
ATMSystem::ATMSystem() : head(nullptr), tail(nullptr), size(0) {
    srand(time(0));
}
...
bool ATMSystem::isEmpty() const {
    return head == nullptr;
}

// Get the size of the list
int ATMSystem::getSize() const {
    return size;
}

// Insert account using AccountNode object (simplified argument)
void ATMSystem::insertAccount(AccountNode account) {
...
bool ATMSystem::deleteAccount(int accountNumber) {
...
AccountNode* ATMSystem::searchAccount(int accountNumber) const {
    AccountNode* current = head;
...
AccountNode* ATMSystem::searchAccountByPin(int pinCode) const {
    AccountNode* current = head;
    while (current != nullptr) {
...
void ATMSystem::displayAllAccounts() const {
    if (isEmpty()) {
...
int ATMSystem::generateAccountNumber() {
    return 10000 + rand() % 90000;
}

// Generate random PIN
int ATMSystem::generatePIN() {
    return 1000 + rand() % 9000;
}

// Validate initial deposit
bool ATMSystem::validateInitialDeposit(float amount) const {
    return amount >= 5000.0;
}

// Registration Module
void ATMSystem::registrationModule() {
...
// Balance Inquiry
void ATMSystem::balanceInquiry(AccountNode* account) {
...
// Withdraw
void ATMSystem::withdraw(AccountNode* account) {
    cout << "\n=== WITHDRAW ===" << endl;
    float amount;
...
// Deposit
void ATMSystem::deposit(AccountNode* account) {
    cout << "\n=== DEPOSIT ===" << endl;
    float amount;
...
// Fund Transfer
void ATMSystem::fundTransfer(AccountNode* fromAccount) {
    cout << "\n=== FUND TRANSFER ===" << endl;
    int targetAccount;
    float amount;
...
// Change PIN
void ATMSystem::changePIN(AccountNode* account) {
    cout << "\n=== CHANGE PIN ===" << endl;
    int newPIN;
...
// Transaction Module
void ATMSystem::transactionModule() {
...
// Display Main Menu
void ATMSystem::displayMainMenu() {
